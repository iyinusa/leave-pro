...
class Sms extends CI_Model {
    ...
    function send_sms($message = '' , $reciever_phone = '') {
        $active_sms = $this->db->get_where('settings' , array(
            'type' => 'active_sms_service'
        ))->row()->description;
        if ($active_sms == '' || $active_sms == 'disabled')
            return;

        if ($active_sms == 'bulksmsng') {
            $this->send_bulksmsng($message , $reciever_phone );
        }
        if ($active_sms == 'clickatell') {
            $this->send_clickatell($message , $reciever_phone );
        }
        if ($active_sms == 'twilio') {
            $this->send_twilio($message , $reciever_phone );
        }
        if ($active_sms == 'msg91') {
            $this->send_msg91($message , $reciever_phone );
        }
    }

    // SEND SMS VIA BULK SMS NG API
    function send_bulksmsng($message = '' , $reciever_phone = '') {
        $sender_id = $this->db->get_where('settings' , array(
            'type' => 'bulksmsng_sender'
        ))->row()->description;

        $token = $this->db->get_where('settings' , array(
            'type' => 'bulksmsng_key'
        ))->row()->description;
        ...
        if($sender_id) {
            ...
            $curl_data = array('api_token'=>$token, 'from'=>$sender_id, 'to'=>$reciever_phone, 'body'=>$message);
    ...
    function send_clickatell($message = '' , $reciever_phone = '') {
    ...
    function send_twilio($message = '' , $reciever_phone = '') {
    ...
    //SMS via msg91
    function send_msg91($message = '' , $reciever_phone = '') {
